...
class Attention(nn.Cell):
    ...
    def construct(self, _x: mindspore.Tensor, start_pos: int,
        ...
        cache_k = cache_k.astype(x_q.dtype)
        cache_v = cache_v.astype(x_q.dtype)

        cache_k[:bsz, start_pos : start_pos + seqlen] = x_k
        cache_v[:bsz, start_pos : start_pos + seqlen] = x_v
...
class TransformerBlock(nn.Cell):
    ...
    def construct(self, _x: mindspore.Tensor, start_pos: int,
                freqs_cis: mindspore.Tensor, mask: Optional[mindspore.Tensor], n_layer: int):
        _h = _x + self.attention(self.attention_norm(_x), start_pos, freqs_cis, mask, n_layer)
